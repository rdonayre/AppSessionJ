	...
	this.each(function() {
		...
		if (!jQuery.datePicker.isInited(this)) {
			...
			if(a && a.inputClick){
				...
				/*
				calBut = jQuery("<a>").attr({'href':'javascript:;', 'class':'date-picker', 'title':chooseDate})
					.append("<span>" + chooseDate + "</span>");
				*/
				calBut = jQuery("<a>").attr({'href':'javascript:;', 'class':'date-picker'})
					.append(chooseDate);
			}
			/*var ap = jQuery("<div>").attr({'class':'popup-calendar'});
			var ccc = jQuery("<div>").attr({'class':'date-picker-holder'}).append(calBut);
			jQuery(this).after(ccc);*/
			jQuery(this).wrap(
