...
(function(c) {
	...
	c.extend(c.expr[":"], {
		...
		tabbable : function(k) {
			...
			function j(m) {
				return !(c(m).is(":hidden") || c(m).parents(":hidden").length)
			}
			return (k.tabIndex >= 0
					&& (("a" == l && k.href) || (/input|select|textarea|button/
							.test(l)
							&& "hidden" != k.type && !k.disabled)) && j(k))
		}
	});
	function g(m, n, o, l) {
		function k(q) {
	...
	c.widget.prototype = {
		...
		_trigger : function(k, l, m) {
			var n = this.options[k], j = (k == this.widgetEventPrefix ? k
					: this.widgetEventPrefix + k);
			l = c.Event(l);
			l.type = j;
			this.element.trigger(l, m);
			return !(c.isFunction(n) && n.call(this.element[0], l, m) === false || l
					.isDefaultPrevented())
		}
	};
	c.widget.defaults = {
