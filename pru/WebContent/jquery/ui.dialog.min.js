...
(function(b) {
					...
					"ui.dialog",
					{
						_init : function() {
							...
							var l = this, m = this.options, j = m.title
									|| "&nbsp;", d = b.ui.dialog
									.getTitleId(this.element), k = (this.uiDialog = b("<div/>"))
									...
									.keydown(
											function(n) {
												(m.closeOnEscape
														&& n.keyCode
														&& n.keyCode == b.ui.keyCode.ESCAPE && l
														.close())
											}).attr({
										role : "dialog",
										"aria-labelledby" : d
										...
										l.moveToTop()
									}), f = this.element.show().removeAttr(
									"title").addClass(
									"ui-dialog-content ui-widget-content")
									.appendTo(k), e = (this.uiDialogTitlebar = b("<div></div>"))
									.addClass(
											"ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix")
									.prependTo(k), i = b('<a href="#"/>')
									.addClass(
									...
									.attr("role", "button").hover(function() {
										...
										l.close();
										return false
									}).appendTo(e), g = (this.uiDialogTitlebarCloseText = b("<span/>"))
									.addClass("ui-icon ui-icon-closethick")
									.text(m.closeText).appendTo(i), c = b(
									"<span/>").addClass("ui-dialog-title")
									.attr("id", d).html(j).prependTo(e), h = (this.uiDialogButtonPane = b("<div></div>"))
									.addClass(
							...
							(m.draggable && b.fn.draggable && this
									._makeDraggable());
							(m.resizable && b.fn.resizable && this
									._makeResizable());
							this._createButtons(m.buttons);
							this._isOpen = false;
							(m.bgiframe && b.fn.bgiframe && k.bgiframe());
						...
						close : function() {
							...
							(this.overlay && this.overlay.destroy());
							this.uiDialog.hide(this.options.hide).unbind(
									"keypress.ui-dialog");
							this._trigger("close");
						...
						moveToTop : function(f) {
							...
							b(".ui-dialog:visible").each(
									function() {
										e = Math.max(e, parseInt(b(this).css(
												"z-index"), 10)
												|| d.zIndex)
									});
						...
						open : function() {
							...
							this.overlay = this.options.modal ? new b.ui.dialog.overlay(
									this)
									: null;
							(this.uiDialog.next().length && this.uiDialog
									.appendTo("body"));
							this._size();
							this._position(this.options.position);
							this.uiDialog.show(this.options.show);
							this.moveToTop(true);
							(this.options.modal && this.uiDialog.bind(
									"keypress.ui-dialog", function(e) {
										if (e.keyCode != b.ui.keyCode.TAB) {
											return
										}
										var d = b(":tabbable", this), f = d
												.filter(":first")[0], c = d
												.filter(":last")[0];
						...
						_makeDraggable : function() {
							...
							this.uiDialog.draggable({
								...
								start : function() {
									(d.dragStart && d.dragStart.apply(
											c.element[0], arguments))
								},
								drag : function() {
									(d.drag && d.drag.apply(c.element[0],
											arguments))
								},
								stop : function() {
									(d.dragStop && d.dragStop.apply(
											c.element[0], arguments));
									b.ui.dialog.overlay.resize()
								}
							})
							...
							this.uiDialog.resizable(
									{
										...
										start : function() {
											(e.resizeStart && e.resizeStart
													.apply(c.element[0],
															arguments))
										},
										resize : function() {
											(e.resize && e.resize.apply(
													c.element[0], arguments))
										},
										handles : d,
										stop : function() {
											(e.resizeStop && e.resizeStop
													.apply(c.element[0],
															arguments));
											b.ui.dialog.overlay.resize()
										}
						...
						_position : function(h) {
							var d = b(window), e = b(document), f = e
									.scrollTop(), c = e.scrollLeft(), g = f;
							if (b.inArray(h, [ "center", "top", "right",
									"bottom", "left" ]) >= 0) {
								h = [
										h == "right" || h == "left" ? h
												: "center",
						...
						_setData : function(d, e) {
							...
							case "resizable":
								...
								break;
							case "title":
								b(".ui-dialog-title", this.uiDialogTitlebar)
										.html(e || "&nbsp;");
								break;
							case "width":
						...
						_size : function() {
							...
							this.element.css({
								minHeight : d.minHeight - c,
								height : d.height == "auto" ? "auto" : d.height
										- c
							})
						}
					});
	...
	b.extend(b.ui.dialog.overlay, {
		...
		create : function(d) {
			if (this.instances.length === 0) {
				...
				b(document).bind(
						...
						function(e) {
							(d.options.closeOnEscape && e.keyCode
									&& e.keyCode == b.ui.keyCode.ESCAPE && d
									.close())
						});
				b(window).bind("resize.dialog-overlay",
						b.ui.dialog.overlay.resize)
			}
			var c = b("<div></div>").appendTo(document.body).addClass(
					"ui-dialog-overlay").css(b.extend({
				borderWidth : 0,
				margin : 0,
				padding : 0,
				position : "absolute",
		...
		destroy : function(c) {
			...
			if (this.instances.length === 0) {
				b("a, :input").add([ document, window ]).unbind(
						".dialog-overlay")
			}
			c.remove()
		},
